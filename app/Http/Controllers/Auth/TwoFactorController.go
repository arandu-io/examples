...
// MarshalJSON keeps the password-state fingerprint out of diagnostics while
// retaining the non-secret identity needed to debug a pending challenge.
func (p pendingSignIn) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		Tenant: p.Tenant, UserID: p.UserID,
		PasswordFingerprint: redacted(p.PasswordFingerprint), Remember: p.Remember,
	})
}

// LogValue enforces the same redaction for structured logs.
func (p pendingSignIn) LogValue() slog.Value {
	return slog.GroupValue(
		slog.String("tenant", p.Tenant), slog.String("user_id", p.UserID),
		slog.String("password_fingerprint", redacted(p.PasswordFingerprint)),
...
func (m *Module) writePending(w http.ResponseWriter, u models.User, remember bool) error {
	...
	// the one seam that must retain the real fingerprint.
	payload, err := json.Marshal(map[string]any{
		"tenant": u.TenantID, "user_id": u.ID,
		"password_fingerprint": u.PasswordFingerprint(), "remember": remember,
	})
	if err != nil {
		return err
	}
...
func (m *Module) beginTwoFactorSetup(w http.ResponseWriter, r *http.Request) {
	...
	m.screen(w, r, "auth.two-factor.setup", AuthPage{
		...
		QRCodeSVG: template.HTML(svg), SecretKey: otp.EncodeSecret(provisioning.Secret),
	})
}
