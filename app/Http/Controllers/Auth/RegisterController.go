...
func (in registrationInput) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		Password: redacted(in.Password), PasswordConfirmation: redacted(in.PasswordConfirmation),
	})
}
...
func (in registrationInput) LogValue() slog.Value {
	return slog.GroupValue(
		...
		slog.String("password_confirmation", redacted(in.PasswordConfirmation)),
	)
}
...
func (m *Module) doRegister(w http.ResponseWriter, r *http.Request) {
	...
	if len([]rune(in.Password)) < security.MinPasswordLen {
